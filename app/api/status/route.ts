...
export async function GET() {
  try {
    // Buscar status de todos os serviços
    ...
    } else if (statuses.some((s: ServiceStatus) => s === 'unstable')) {
    ...
    const statusCounts = {
      ...
      unstable: statuses.filter((s: ServiceStatus) => s === 'unstable').length,
    }

    // Verificar se há erros nos últimos 10 minutos
    const servicesWithErrors = services.filter(
      (s: { hasErrorLast10Minutes: boolean }) => s.hasErrorLast10Minutes
    )

    // Formatar resposta compatível com n8n
    const response = {
      success: true,
      ...
      services: services.map((s: { 
        ...
        name: s.name,
        url: s.url,
        status: s.status === 'operational' ? 'online' : s.status === 'unavailable' ? 'offline' : s.status,
